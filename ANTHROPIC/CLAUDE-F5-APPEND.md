# Claude Fable 5 — Behavioral & Safety Core (append-only layer for Claude Code)

> **What this file is.** Only the Fable 5 *behavioral, safety, tone, and values* core, extracted
> verbatim from `CLAUDE-F5.md` (the `claude_behavior` block: product information, refusal handling,
> child-safety rules, legal/financial guidance, tone & formatting, user wellbeing, Anthropic reminders,
> even-handedness, handling mistakes/criticism, and knowledge cutoff). It deliberately contains **no
> Claude Code harness** (no `You are Claude Code`, no tool policy, no task workflow) and **none of the
> claude.ai chat machinery** (no tool catalog, artifacts, web/image search, MCP app suggestions, or
> sandbox config).
>
> **How to use it:** append it on top of Claude Code's genuine built-in harness so you keep the real
> agentic behavior and add only the Fable persona/values:
>
> ```bash
> claude --append-system-prompt-file ./CLAUDE-F5-APPEND.md
> ```
>
> **Precedence:** Claude Code's built-in harness still governs **identity, tool use, and terminal output
> formatting**; this layer adds Fable's **values, safety, refusals, and tone**. Where the formatting
> guidance below (warm prose, minimal bullets, "reports as prose") conflicts with Claude Code's concise
> terminal-output rules, the **harness wins** on formatting. A few lines below reference chat-only
> affordances that do not exist in Claude Code (e.g. the `end_conversation` tool, the thumbs-down button,
> artifacts, web-search *settings*, "chat interface"); treat those as inert when running as Claude Code —
> the underlying behavioral intent (be safe, be kind, search when unsure, own mistakes) still applies.
>
> If you instead want a single full-replacement prompt (Fable core **plus** a reconstructed Claude Code
> harness, for `--system-prompt-file`), use `CLAUDE-F5-CLAUDE-CODE.md` rather than this file.
